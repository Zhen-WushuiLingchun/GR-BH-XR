...
| Native NPGS fork | Accepted for neutral exterior Kerr/Kerr-Newman rays, Kerr `Q=0` disk transfer, geometric polarization transport, the audited approximate dynamic-BBH slice, pre-device dynamic XR/MR contracts, and bounded time-indexed Vulkan playback mechanics | Complete official history is pinned through the GPL fork. Native states pass independent CPU f64 event/direction/disk/polarization and four-dimensional BBH replay gates. The keyframe player validates and rehashes v3 assets, maps two logical endpoints onto three physical slots, prefetches CPU bytes, retires slots against per-frame fences, and interpolates physical texels before shading. Continuing replacement is split into 36 descriptor-hidden cubemap-face uploads; the largest measured 1024/face slice was `4.02 ms`. Initial bracket upload is still synchronous, and no production merger sequence, native OpenXR session, or delivered MR camera frame has passed its final gate. |

The repository suite passed `395` tests on 2026-08-12 after the bounded
...
`Zhen-WushuiLingchun/NPGS@5986b80cb16cc228b581040ad1009cc547e7f522`.
...
- Native NPGS now repeats the runtime-critical manifest, gate, layout, byte,
  ...
  three-slot Vulkan residency with two logical bracket endpoints. Dedicated
  shader variants interpolate event,
  escape-direction, and `m=0/1` disk quantities per texel before visual
  shading. CPU-prefetched replacements upload one cubemap face per rendered
  frame and remain invisible until all 36 role/face slices complete. The
  deterministic four-frame smoke passed `0/1 -> 1/2 -> 2/3`, but no complete
  production merger asset or OpenXR frame-time claim is made.
